...
async function handlePaymentMonetbilSuccess(req, res, client) {
  try {
    ...
    const successMessage = `Félicitations ${user.pseudo} ! Votre paiement pour *${quantity}* ${product.name} - ${variation.name}, a été effectué avec succès. Un livreur vous appellera dans les minutes qui suivent. Ci-joint, votre facture.`;
    ...
    const adminMessage = `Un client (${user.pseudo || user.phoneNumber}) a effectué un achat pour *${quantity}* ${product.name} - ${variation.name}. Veuillez trouver la facture en pièce jointe.`;
